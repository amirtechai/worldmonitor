...
| Pattern | Matches |
| --- | --- |
| `(*.)?xworld.amirtech.ai` | Production + subdomains (`tech.`, `finance.`, etc.) |
| `xworld-*-elie-*.vercel.app` | Vercel preview deploys |
...
- `Content-Type`
- `Authorization`
- `X-XWorld-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
