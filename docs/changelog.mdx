---
title: "Changelog"
description: "All notable changes to XWorld, organized by release."
rss: true
---

All notable changes to XWorld are documented here. Subscribe via [RSS](/changelog/rss.xml) to stay updated.
...
- **CDN cache bypass closed**: `CDN-Cache-Control` header now only emitted for trusted origins (xworld.amirtech.ai, Vercel previews, Tauri). No-origin server-side requests always invoke the edge function so `validateApiKey` runs, preventing a cached 200 from being served to external scrapers.
