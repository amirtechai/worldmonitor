...
description: "XWorld exposes two health endpoints for monitoring data pipeline integrity. Both run on the Vercel Edge Runtime and query Redis (Upstash) to assess system state."
---
...
```bash
# Full health check
curl -s https://xworld.amirtech.ai/api/api/health | jq .

# Compact (problems only)
curl -s "https://xworld.amirtech.ai/api/api/health?compact=1" | jq .

# UptimeRobot / monitoring: check HTTP status code
curl -o /dev/null -s -w "%{http_code}" https://xworld.amirtech.ai/api/api/health
...
```bash
curl -s https://xworld.amirtech.ai/api/api/seed-health \
  -H "Origin: https://xworld.amirtech.ai" | jq .
```
...
STATUS=$(curl -s "https://xworld.amirtech.ai/api/api/health?compact=1")
