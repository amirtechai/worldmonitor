# XWorld Pro — Implementation Roadmap
...
- **Auth**: none — no Clerk, no sessions. Desktop uses manual `XWORLD_API_KEY` in keychain
- **Payments**: none — no Stripe
- **Gating**: UI-only on desktop (6 panels, 3 map layers). No server-side enforcement. `api/_api-key.js` validates against static `XWORLD_VALID_KEYS` env var
- **User dashboard**: none
...
**Description**:
Evaluate and select an authentication provider for XWorld Pro.
...
1. Register `xworld://auth/callback` deep link URI scheme in Tauri config
...
1. **XWorld Pro Monthly** — $X/mo
2. **XWorld Pro Annual** — $X/yr (discount)
3. **XWorld API Starter** — $Y/mo (1,000 req/day, 5 webhook rules)
4. **XWorld API Business** — $Z/mo (50,000 req/day, unlimited webhooks + SLA)
...
1. **Phase A**: validate against BOTH static `XWORLD_VALID_KEYS` AND new entitlements. Log comparison metrics.
...
4. Replace ALL `getSecretState('XWORLD_API_KEY').present` checks with plan context
...
- [ ] All `getSecretState('XWORLD_API_KEY')` references replaced
...
- New sebuf proto service: `xworld/equity/v1/`
...
3. **Telegram** — Bot API. Create `@XWorldBot`. User starts conversation, store `chat_id`.
...
**Description**:
Pro users should NOT need to configure individual API keys for Finnhub, FRED, ACLED, etc. A single XWorld Pro subscription gives access to all 24 services.

**Implementation**:

- Server-side: pro requests use XWorld's own upstream API keys (already configured as env vars)
...
5. **Desktop auth + Tauri WKWebView**: Known limitations. PKCE flow with `xworld://` deep link callback.
