...
**Purpose:** Reference document for extending XWorld's finance hub with TradingView Screener data.
...
The TypeScript library is the one relevant for XWorld. It has full parity with the Python version (41/41 operations pass) plus a self-hosted REST server option.
...
XWorld's data flow: `Railway seeds Redis → Vercel reads Redis → Frontend RPC`
...
```typescript
// server/xworld/market/v1/list-tv-stock-screener.ts
...
### XWorld-Specific Recommendations
...
| **No auth initially** | Delayed data is fine for XWorld's use case |
...
- [ ] Create `proto/xworld/market/v1/list_tv_stock_screener.proto`
- [ ] Create `server/xworld/market/v1/list-tv-stock-screener.ts`
