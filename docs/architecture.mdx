...
description: "Design principles, intelligence tradecraft, algorithmic decisions, and implementation patterns behind XWorld."
---

> **Looking for the system reference?** See [`ARCHITECTURE.md`](https://github.com/koala73/xworld/blob/main/ARCHITECTURE.md) for deployment topology, directory layout, caching layers, CI/CD, and contributor how-to guides.
...
XWorld is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
All five dashboard variants (XWorld, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:
...
| `xworld.amirtech.ai` (default) | `full` |

On the desktop app, the variant is stored in `localStorage['xworld-variant']` and can be switched without rebuilding. The variant selector in the header bar navigates between deployed domains on the web or toggles the localStorage value on desktop.
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `xworld.amirtech.ai`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
...
Transactions are sampled at 10% to balance observability with cost. Release tracking (`xworld@{version}`) enables regression detection across deployments.
