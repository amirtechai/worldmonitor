# XWorld — API Reference

> Comprehensive reference for all Vercel Edge Function endpoints powering the XWorld intelligence dashboard.

**Base URL**: All endpoints are relative to `/api/` (e.g., `https://xworld.amirtech.ai/api/earthquakes`).
...
XWorld exposes **60+ serverless endpoints** deployed as **Vercel Edge Functions** (unless noted otherwise). Every endpoint:
...
| # | Pattern | Matches |
|---|---------|---------|
| 1 | `xworld\.app$` | `https://xworld.amirtech.ai` |
| 2 | `\.xworld\.app$` | `https://*.xworld.amirtech.ai` |
...
XWorld uses a **multi-tier caching strategy** to minimize upstream API calls, reduce latency, and stay within third-party rate limits.
