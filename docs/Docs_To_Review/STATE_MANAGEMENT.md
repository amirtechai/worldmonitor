...
XWorld is an AI-powered real-time global intelligence dashboard built with **vanilla TypeScript** — no framework, no reactive stores. All state is managed manually through class properties, `localStorage`, `IndexedDB`, and URL query parameters.
...
Stored in `localStorage` under `xworld-panels` as `Record<string, PanelConfig>`. Includes per-panel `enabled` state, `name`, and `priority`. Controlled by the variant config and user overrides.
...
| Key | Type | Purpose |
|-----|------|---------|
| `xworld-panels` | `Record<string, PanelConfig>` | Per-panel enabled/name/priority |
| `xworld-monitors` | `Monitor[]` | Color/label configs for monitors |
| `xworld-layers` | `MapLayers` | Enabled/disabled map layer toggles |
| `xworld-disabled-feeds` | `string[]` | User-disabled news feed sources |
...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('xworld-theme', 'light')
...
- **Toggles key:** `xworld-runtime-feature-toggles` in `localStorage`
...
Complete reference of every `localStorage` key used by XWorld:

| Key | Purpose | Format | Source |
|-----|---------|--------|--------|
| `xworld-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `xworld-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `xworld-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
| `xworld-panels` | Panel enabled/config state | `Record<string, PanelConfig>` (JSON) | `STORAGE_KEYS.panels` |
| `xworld-monitors` | Monitor color/label configs | `Monitor[]` (JSON) | `STORAGE_KEYS.monitors` |
| `xworld-layers` | Map layer toggles | `MapLayers` (JSON) | `STORAGE_KEYS.mapLayers` |
| `xworld-disabled-feeds` | User-disabled news sources | `string[]` (JSON) | `STORAGE_KEYS.disabledFeeds` |
| `xworld-runtime-feature-toggles` | Desktop feature toggles | `Record<RuntimeFeatureId, boolean>` (JSON) | `src/services/runtime-config.ts` |
| `xworld-persistent-cache:{key}` | Persistent data cache entries | `CacheEnvelope<T>` (JSON) | `src/services/persistent-cache.ts` |
| `wm-update-dismissed-{version}` | Dismissed update notifications | `'1'` | `App.ts` |
| `xworld-panel-order-v1.9` | Panel order migration flag | `'done'` | `App.ts` (one-time migration) |
| `xworld-tech-insights-top-v1` | Tech variant migration flag | `'done'` | `App.ts` (one-time migration) |
