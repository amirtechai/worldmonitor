...
> **XWorld** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-xworldio)
5. [Tech Variant Panels](#5-tech-variant-panels-techxworldio)
6. [Finance Variant Panels](#6-finance-variant-panels-financexworldio)
...
XWorld uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
|---------|--------|-------|-------------|
| `full` | xworld.io | Geopolitical intelligence, OSINT, defense | 37 |
| `tech` | tech.xworld.io | Technology, AI/ML, startups, VC | 34 |
| `finance` | finance.xworld.io | Markets, trading, macro, commodities | 29 |
...
export const STORAGE_KEYS = {
  panels:        'xworld-panels',          // Panel visibility toggles
  monitors:      'xworld-monitors',         // Monitor keyword configs
  mapLayers:     'xworld-layers',           // Map layer toggles
  disabledFeeds: 'xworld-disabled-feeds',   // Per-source feed disabling
} as const;
```
...
|-----|---------|------------|
| `xworld-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `xworld-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `xworld-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `xworld-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (xworld.io)
...
## 5. Tech Variant Panels (tech.xworld.io)
...
## 6. Finance Variant Panels (finance.xworld.io)
...
|---------|-----------------|--------|----------------|:--------------:|
| Panel visibility | `xworld-panels` | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS` | ✅ |
| Panel ordering | `panel-order` | `string[]` JSON | Config declaration order | ✅ |
| Panel sizes/spans | `xworld-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
| Map layer toggles | `xworld-layers` | `MapLayers` JSON | `DEFAULT_MAP_LAYERS` | ✅ |
| Monitor keywords | `xworld-monitors` | `Monitor[]` JSON | `[]` | ✅ |
| Disabled sources | `xworld-disabled-feeds` | `string[]` JSON | `[]` | ✅ |
| Active variant | `xworld-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`xworld-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:

```typescript
if (storedVariant !== currentVariant) {
  localStorage.setItem('xworld-variant', currentVariant);
...
This ensures users switching between variant domains (e.g. from xworld.io to tech.xworld.io) get a clean default experience for the new variant.
...
Step-by-step guide to adding a new panel to XWorld.
...
sequenceDiagram
    ...
    App->>localStorage: saveToStorage('xworld-panels', panelSettings)
    ...
    App->>localStorage: loadFromStorage('xworld-panels')
