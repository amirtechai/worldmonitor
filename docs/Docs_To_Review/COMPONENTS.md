# Component Documentation — XWorld
...
XWorld's UI is built entirely with **vanilla TypeScript** — no React, Vue, or
...
`localStorage['xworld-panel-spans']` as a JSON map of `{ [id]: height }`.
...
XWorld ships two independent map renderers and an adapter that picks the
...
Recent selections stored in `localStorage['xworld_recent_searches']`
(most recent 10).
