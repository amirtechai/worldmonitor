...
| **IndexedDB** | `xworld_db` | Client-side storage for playback snapshots and temporal baseline data. |
...
XWorld ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.

| Variant | Domain | Focus |
|---|---|---|
| `full` | xworld.amirtech.ai | Geopolitics, military, OSINT, conflicts, markets |
| `tech` | tech.xworld.amirtech.ai | AI/ML, startups, cybersecurity, developer tools |
| `finance` | finance.xworld.amirtech.ai | Markets, trading, central banks, macro indicators |
...
```
localStorage('xworld-variant')  →  import.meta.env.VITE_VARIANT  →  default 'full'
```
...
graph TD
    ...
    subgraph LayerConfig["Layer Configuration"]
        ...
        UserPref["localStorage overrides<br/>(xworld-layers)"]
...
2. **User localStorage** — Stored under the key `xworld-layers`. Users toggle layers in the map controls UI, and their preferences persist across sessions.
...
XWorld employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
graph TD
    ...
    subgraph Tier5["Tier 5: Persistent Cache"]
        ...
        LSFallback["localStorage fallback<br/>prefix: xworld-persistent-cache:"]
        PC --> TauriInvoke
        PC --> LSFallback
    end
...
The `xworld_db` IndexedDB database contains two object stores:
...
On desktop, `getPersistentCache()` and `setPersistentCache()` attempt Tauri IPC invocations (`read_cache_entry` / `write_cache_entry`) first, which store data on the OS filesystem via the Rust backend. If the Tauri call fails (or in web mode), the module falls back to `localStorage` with the prefix `xworld-persistent-cache:`.
...
**2. Feature Toggles** — 14 `RuntimeFeatureId` values stored in localStorage under the key `xworld-runtime-feature-toggles`:
...
XWorld runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
XWorld uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
