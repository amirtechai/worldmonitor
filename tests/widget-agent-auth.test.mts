...
const originalValidKeys = process.env.XWORLD_VALID_KEYS;
...
describe('widget-agent unified tester key auth', () => {
  ...
  before(async () => {
    ...
    process.env.XWORLD_VALID_KEYS = 'browser-test-key';
  ...
  after(() => {
    ...
    if (originalValidKeys == null) delete process.env.XWORLD_VALID_KEYS;
    else process.env.XWORLD_VALID_KEYS = originalValidKeys;
  });

  it('accepts X-XWorld-Key and upgrades relay request to pro', async () => {
    const res = await handler(new Request('https://xworld.amirtech.ai/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://xworld.amirtech.ai',
        'Content-Type': 'application/json',
        'X-XWorld-Key': 'browser-test-key',
      },
    ...
    assert.equal(call.arguments[0], 'https://proxy.xworld.amirtech.ai/widget-agent');
    ...
    assert.equal(headers.get('X-XWorld-Key'), null);
  ...
  it('falls back to legacy tester keys when X-XWorld-Key is invalid', async () => {
    const res = await handler(new Request('https://xworld.amirtech.ai/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://xworld.amirtech.ai',
        'Content-Type': 'application/json',
        'X-XWorld-Key': 'wrong-key',
  ...
  it('rejects invalid X-XWorld-Key before relay fetch', async () => {
    const res = await handler(new Request('https://xworld.amirtech.ai/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://xworld.amirtech.ai',
