...
const originalKeys = process.env.XWORLD_VALID_KEYS;

afterEach(() => {
  if (originalKeys == null) delete process.env.XWORLD_VALID_KEYS;
  else process.env.XWORLD_VALID_KEYS = originalKeys;
});
...
describe('premium gateway API key enforcement', () => {
  it('requires credentials for premium endpoints regardless of origin', async () => {
    ...
    process.env.XWORLD_VALID_KEYS = 'real-key-123';

    // Trusted browser origin without credentials — 401 (no API key, no bearer token)
    const browserNoKey = await handler(new Request('https://xworld.amirtech.ai/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: { Origin: 'https://xworld.amirtech.ai' },
    }));
    assert.equal(browserNoKey.status, 401);

    const resilienceScoreNoKey = await handler(new Request('https://xworld.amirtech.ai/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: { Origin: 'https://xworld.amirtech.ai' },
    }));
    assert.equal(resilienceScoreNoKey.status, 401);

    const resilienceRankingNoKey = await handler(new Request('https://xworld.amirtech.ai/api/resilience/v1/get-resilience-ranking', {
      headers: { Origin: 'https://xworld.amirtech.ai' },
    }));
    ...
    const browserWithKey = await handler(new Request('https://xworld.amirtech.ai/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://xworld.amirtech.ai',
        'X-XWorld-Key': 'real-key-123',
      },
    }));
    assert.equal(browserWithKey.status, 200);

    const resilienceScoreWithKey = await handler(new Request('https://xworld.amirtech.ai/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://xworld.amirtech.ai',
        'X-XWorld-Key': 'real-key-123',
      },
    ...
    const resilienceRankingWithKey = await handler(new Request('https://xworld.amirtech.ai/api/resilience/v1/get-resilience-ranking', {
      headers: {
        Origin: 'https://xworld.amirtech.ai',
        'X-XWorld-Key': 'real-key-123',
      },
    ...
    const publicAllowed = await handler(new Request('https://xworld.amirtech.ai/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://xworld.amirtech.ai' },
    }));
...
describe('premium gateway bearer token auth', () => {
  ...
  it('valid bearer token resolves userId but entitlement check still applies', async () => {
    ...
    const res = await handler(new Request('https://xworld.amirtech.ai/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://xworld.amirtech.ai',
  ...
  it('public routes are unaffected by absence of auth header', async () => {
    const res = await handler(new Request('https://xworld.amirtech.ai/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://xworld.amirtech.ai' },
    }));
  ...
  it('rejects free bearer token on resilience premium endpoints → 403', async () => {
    ...
    const scoreRes = await handler(new Request('https://xworld.amirtech.ai/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://xworld.amirtech.ai',
        Authorization: `Bearer ${token}`,
      },
    ...
    const rankingRes = await handler(new Request('https://xworld.amirtech.ai/api/resilience/v1/get-resilience-ranking', {
      headers: {
        Origin: 'https://xworld.amirtech.ai',
        Authorization: `Bearer ${token}`,
      },
