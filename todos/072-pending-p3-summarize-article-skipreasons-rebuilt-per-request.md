...
## Problem Statement
`server/xworld/news/v1/summarize-article.ts` lines 65-69 builds a `skipReasons` object on every invocation including hot-path cache hits:
```ts
...
## Technical Details
- File: `server/xworld/news/v1/summarize-article.ts:65-69, 89`
