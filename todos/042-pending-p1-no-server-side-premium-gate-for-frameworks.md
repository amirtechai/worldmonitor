...
## Findings
- **`server/xworld/intelligence/v1/get-country-intel-brief.ts`** — reads `req.framework`, truncates to 2000 chars, passes to `callLlm` as `systemAppend`. No entitlement check.
- **`server/xworld/intelligence/v1/deduct-situation.ts`** — same pattern, no entitlement check.
- **`server/xworld/news/v1/summarize-article.ts`** — reads `req.systemAppend`, no entitlement check.
...
- Flagged by: security-sentinel, architecture-strategist, learnings-researcher (xworld-pro-panel-gating skill)
...
## Technical Details
- Files: `server/xworld/intelligence/v1/get-country-intel-brief.ts`, `server/xworld/intelligence/v1/deduct-situation.ts`, `server/xworld/news/v1/summarize-article.ts`
- PR: koala73/xworld#2380
- Reference: xworld-pro-panel-gating skill (4-layer checklist)
