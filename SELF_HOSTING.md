...
Run the full XWorld stack locally with Docker/Podman.
...
*/30 * * * * cd /path/to/xworld && ./scripts/run-seeders.sh >> /tmp/wm-seeders.log 2>&1
```
...
export UPSTASH_REDIS_REST_TOKEN=xworld-local-token
...
|-----------|---------|------|
| `xworld` | nginx + Node.js API (supervisord) | 3000 → 8080 |
| `xworld-redis` | Data store | 6379 (internal) |
| `xworld-redis-rest` | Upstash-compatible REST proxy | 8079 |
| `xworld-ais-relay` | Live vessel tracking WebSocket | 3004 (internal) |
...
# Full Docker image
docker build -t xworld:latest -f Dockerfile .
...
| 🔴 nginx won't start | Check `podman logs xworld` — likely missing `gettext` package |
...
| 🚢 No vessel data | Set `AISSTREAM_API_KEY` in both `xworld` and `ais-relay` services |
