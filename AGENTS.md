...
Agent entry point for XWorld. Read this first, then follow links for depth.
...
│   └── xworld/       # Domain handlers (mirrors proto service structure)
├── proto/                  # Protobuf definitions (sebuf framework)
│   ├── buf.yaml            # Buf configuration
│   └── xworld/       # Service definitions with HTTP annotations
...
- `server/xworld/<domain>/` has RPC handlers matching proto services
...
1. Define proto message in `proto/xworld/<domain>/`
2. Add RPC with `(sebuf.http.config)` annotation
3. Run `make generate`
4. Create handler in `server/xworld/<domain>/`
