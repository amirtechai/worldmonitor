...
const DEFAULT_WEB_API_URL = 'https://xworld.amirtech.ai/api';
...
function isXWorldWebHost(hostname: string): boolean {
  return hostname === 'xworld.amirtech.ai'
    || hostname === 'www.xworld.amirtech.ai'
    || hostname.endsWith('.xworld.amirtech.ai');
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://xworld.amirtech.ai';
  return '';
}
...
const APP_HOSTS = new Set([
  'xworld.amirtech.ai',
  'www.xworld.amirtech.ai',
  'tech.xworld.amirtech.ai',
  'api.xworld.amirtech.ai',
  'localhost',
  '127.0.0.1',
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.xworld.amirtech.ai');
  } catch {
    return false;
  }
}
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (allowCloudFallback && !isKeyFreeApiTarget(target)) {
      try {
        ...
        const wmKeyState = getSecretState('XWORLD_API_KEY');
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        const wmKeyValue = getRuntimeConfigSnapshot().secrets['XWORLD_API_KEY']?.value;
        if (wmKeyValue) {
          cloudHeaders.set('X-XWorld-Key', wmKeyValue);
        }
      }
...
const ALLOWED_REDIRECT_HOSTS = /^https:\/\/([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*xworld\.app(:\d+)?$/;
...
export function installWebApiRedirect(): void {
   ...
   *   2. XWORLD_API_KEY from runtime config → X-XWorld-Key
   *   3. Tester key (xw-pro-key / wm-widget-key) → X-XWorld-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    if (headers.has('Authorization') || headers.has('X-XWorld-Key')) return init;
    // XWORLD_API_KEY from env or runtime config
    try {
      const { getRuntimeConfigSnapshot } = await import('@/services/runtime-config');
      const wmKey = getRuntimeConfigSnapshot().secrets['XWORLD_API_KEY']?.value;
      if (wmKey) {
        headers.set('X-XWorld-Key', wmKey);
        return { ...init, headers };
      }
    } catch { /* runtime-config unavailable — fall through */ }
    // Tester key (xw-pro-key / wm-widget-key): forward as API key header.
    ...
    if (testerKey) {
      headers.set('X-XWorld-Key', testerKey);
      return { ...init, headers };
    }
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://xworld.amirtech.ai/api/api/...) — just inject auth.
