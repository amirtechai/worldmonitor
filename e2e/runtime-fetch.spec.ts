...
test.describe('desktop runtime routing guardrails', () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        webHost: runtime.detectDesktopRuntime({
          ...
          userAgent: 'Mozilla/5.0',
          locationProtocol: 'https:',
          locationHost: 'xworld.amirtech.ai',
          locationOrigin: 'https://xworld.amirtech.ai',
        }),
      };
    });
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('xworld.amirtech.ai/api/fred-data')) {
        ...
        if (url.includes('xworld.amirtech.ai/api/stablecoin-markets')) {
      ...
      await runtimeConfig.setSecretValue('XWORLD_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, 'wm_test_key_1234567890abcdef');
      ...
      try {
        ...
        await runtimeConfig.setSecretValue('XWORLD_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, '');
      }
    });
    ...
    expect(result.calls.some((url) => url.includes('xworld.amirtech.ai/api/fred-data'))).toBe(true);
    expect(result.calls.some((url) => url.includes('127.0.0.1:46123/api/stablecoin-markets'))).toBe(true);
    expect(result.calls.some((url) => url.includes('xworld.amirtech.ai/api/stablecoin-markets'))).toBe(true);
  });
  ...
  test('runtime fetch patch never sends local-only endpoints to cloud', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('xworld.amirtech.ai/api/local-env-update')) {
          return responseJson({ leaked: true }, 200);
        }
        if (url.includes('xworld.amirtech.ai/api/local-validate-secret')) {
          return responseJson({ leaked: true }, 200);
        }
    ...
    expect(result.calls.some((url) => url.includes('xworld.amirtech.ai/api/local-env-update'))).toBe(false);
    expect(result.calls.some((url) => url.includes('xworld.amirtech.ai/api/local-validate-secret'))).toBe(false);
  });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const previousTauri = globalWindow.__TAURI__;
      const releaseUrl = 'https://github.com/koala73/xworld/releases/latest';
    ...
    expect(result.macArm).toBe('https://xworld.amirtech.ai/api/download?platform=macos-arm64&variant=full');
    expect(result.windowsX64).toBe('https://xworld.amirtech.ai/api/download?platform=windows-exe&variant=full');
    expect(result.linuxFallback).toBe('https://github.com/koala73/xworld/releases/latest');
  });
  ...
  test('cloud fallback blocked without XWorld API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const cloudCalls = calls.filter(u => u.includes('xworld.amirtech.ai'));
  ...
  test('cloud fallback allowed with valid XWorld API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('xworld.amirtech.ai') && init?.headers) {
          const h = new Headers(init.headers);
          const wmKey = h.get('X-XWorld-Key');
          if (wmKey) capturedHeaders['X-XWorld-Key'] = wmKey;
        }

        if (url.includes('127.0.0.1:46123/api/market/v1/test')) {
          throw new Error('ECONNREFUSED');
        }
        if (url.includes('xworld.amirtech.ai/api/market/v1/test')) {
      ...
      await runtimeConfig.setSecretValue('XWORLD_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, testKey);
      ...
      try {
        ...
        return {
          ...
          cloudCalls: calls.filter(u => u.includes('xworld.amirtech.ai')).length,
          wmKeyHeader: capturedHeaders['X-XWorld-Key'] || null,
        };
      } finally {
        window.fetch = originalFetch;
        delete globalWindow.__wmFetchPatched;
